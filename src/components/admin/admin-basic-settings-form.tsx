...
const PROFILE_POST_LINK_DISPLAY_MODE_OPTIONS = [
  { value: "SLUG", label: "slug 模式（/posts/标题-id）" },
  { value: "ID", label: "id 模式（/posts/id）" },
] as const
const PROFILE_POST_SLUG_GENERATION_MODE_OPTIONS = [
...
export function AdminBasicSettingsForm({
  ...
  const {
    ...
    registrationEnabled,
    authPageShowcaseEnabled,
    registrationRequireInviteCode,
    registerInviteCodeEnabled,
    registerInviteCodeHelpEnabled,
    registerInviteCodeHelpTitle,
    registerInviteCodeHelpUrl,
    ...
    registerNicknameMinLength,
    registerNicknameMaxLength,
    registerGenderEnabled,
    registerGenderRequired,
    registerInviterEnabled,
    registerVerificationEmailSubject,
    registerVerificationEmailText,
    registerVerificationEmailHtml,
  ...
  async function uploadSiteIcon(file: File) {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "站点图标上传失败，请稍后再试", "上传失败")
    } finally {
  ...
  return (
    ...
    >
        ...
        {INTERNAL_SETTING_TABS[mode].length > 1 && !subTabRouteSection ? (
          <AdminSettingsSubTabs
            items={INTERNAL_SETTING_TABS[mode].map((tab) => ({
              ...
              label: tab.label,
              ...(subTabRouteSection
                ? { href: getAdminSettingsHref(subTabRouteSection, tab.key) }
            ...
            >
              <div className="space-y-4">
                <div className="rounded-2xl border border-border bg-muted/30 p-4">
                  <p className="text-xs font-medium text-muted-foreground">导航品牌</p>
                  <div className="mt-3 flex items-center gap-3">
                    <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-border bg-background text-sm font-semibold">
                      {(siteLogoText || siteName || "站").slice(0, 1)}
                    </div>
                    <div className="min-w-0">
                      <p className="truncate text-sm font-semibold">
                        {siteLogoText || "未设置 Logo 文案"}
                      </p>
                ...
                <div className="rounded-2xl border border-border bg-muted/20 p-4">
                  <p className="text-xs font-medium text-muted-foreground">品牌副标题</p>
                  <p className="mt-2 text-sm leading-6">
                    {siteSlogan || "未填写 Slogan 时，前台会保持更简洁的品牌展示。"}
                  </p>
                </div>

                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="rounded-2xl border border-dashed border-border bg-background px-4 py-3">
                    <p className="text-xs font-medium text-muted-foreground">图片 Logo</p>
                    <p className="mt-2 text-sm">{siteLogoPath ? "已设置图片 Logo" : "未设置，回退到文字品牌"}</p>
                  </div>
                  <div className="rounded-2xl border border-dashed border-border bg-background px-4 py-3">
                    <p className="text-xs font-medium text-muted-foreground">站点 Icon</p>
                    <p className="mt-2 text-sm">{siteIconPath ? "已设置浏览器标签图标" : "未设置，将使用默认图标"}</p>
                  </div>
                </div>
              </div>
            </SettingsSection>
        ...
        <div className="space-y-4">
          <div className="grid gap-4 xl:grid-cols-2">
            ...
            >
              <div className="grid gap-4 md:grid-cols-2">
                <SettingsSelectField
                  label="帖子链接显示模式"
                  value={postLinkDisplayMode}
                  onChange={(value) => updateDraftField("postLinkDisplayMode", value as "SLUG" | "ID")}
                  options={[...PROFILE_POST_LINK_DISPLAY_MODE_OPTIONS]}
                  description="slug 模式更利于可读性和 SEO；id 模式更短更稳定。"
                />
                <SettingsSelectField
                  label="帖子 slug 生成规则"
                  value={postSlugGenerationMode}
                  onChange={(value) => updateDraftField("postSlugGenerationMode", value as typeof postSlugGenerationMode)}
                  options={[...PROFILE_POST_SLUG_GENERATION_MODE_OPTIONS]}
                  description="仅在 slug 模式下生效，决定新帖 URL 的可读性和唯一性。"
                />
                <SettingsSelectField
                  label="首页帖子列表形式"
                  value={homeFeedPostListDisplayMode}
                  onChange={(value) => updateDraftField("homeFeedPostListDisplayMode", resolveHomeFeedPostListDisplayMode(value))}
                  options={[...PROFILE_HOME_FEED_DISPLAY_MODE_OPTIONS]}
                  description="只影响首页 feed 的普通帖子列表；置顶帖仍保持原来的普通列表样式。"
                />
                <SettingsSelectField
                  label="首页帖子加载方式"
                  value={homeFeedPostListLoadMode}
                  onChange={(value) => updateDraftField("homeFeedPostListLoadMode", value as typeof POST_LIST_LOAD_MODE_PAGINATION | typeof POST_LIST_LOAD_MODE_INFINITE)}
            ...
            <SettingsSection
              ...
              action={(
                <Badge variant={searchEnabled ? "secondary" : "outline"}>
                  {searchEnabled ? "搜索已开启" : "搜索已关闭"}
                </Badge>
              )}
              className="h-full"
            >
              <div className="grid gap-3">
                <SettingsSelectField
                  label="全局左侧导航模式"
                  value={leftSidebarDisplayMode}
                  onChange={(value) => updateDraftField("leftSidebarDisplayMode", value as typeof leftSidebarDisplayMode)}
                  options={[...PROFILE_LEFT_SIDEBAR_DISPLAY_MODE_OPTIONS]}
                  description="默认保持现在的三栏布局；隐藏会移除左侧导航；吸附模式会把左侧导航收进最左侧抽屉，首次默认隐藏。"
                />
                <SettingsToggleField
                  label="首页右侧统计卡片"
                  checked={homeSidebarStatsCardEnabled}
                  onChange={(value) => updateDraftField("homeSidebarStatsCardEnabled", value)}
                  description="展示站点用户、帖子、节点等总体概况。"
                />
                <SettingsToggleField
                  label="首页右侧站点公告"
                  checked={homeSidebarAnnouncementsEnabled}
                  onChange={(value) => updateDraftField("homeSidebarAnnouncementsEnabled", value)}
          ...
          >
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
              <TextField
                label="首页帖子显示数量"
                value={homeFeedPostPageSize}
                onChange={(value) => updateDraftField("homeFeedPostPageSize", value)}
                placeholder="如 35"
                type="number"
              ...
              <TextField
                label="今日热帖显示数量"
                value={homeSidebarHotTopicsCount}
                onChange={(value) => updateDraftField("homeSidebarHotTopicsCount", value)}
                placeholder="如 5"
                type="number"
              ...
              <TextField
                label="帖子相关主题显示数量"
                value={postSidebarRelatedTopicsCount}
                onChange={(value) => updateDraftField("postSidebarRelatedTopicsCount", value)}
                placeholder="如 5"
                type="number"
        ...
        <div className="space-y-4">
          <div className="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)]">
            ...
            <SettingsSection
              ...
              action={(
                <Badge variant={footerBrandingVisible ? "secondary" : "outline"}>
                  {footerBrandingVisible ? "附加信息显示中" : "仅展示版权文案"}
                </Badge>
              )}
              className="h-full"
            >
        ...
        <>
          <div className="rounded-[24px] border border-border p-5 space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
              ...
              <AdminBooleanSelectField label="显示登录/注册页左侧内容" checked={authPageShowcaseEnabled} onChange={(value) => updateDraftField("authPageShowcaseEnabled", value)} />
            ...
            {registerInviteCodeEnabled ? (
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                <AdminBooleanSelectField label="邀请码输入框链接开关" checked={registerInviteCodeHelpEnabled} onChange={(value) => updateDraftField("registerInviteCodeHelpEnabled", value)} />
                <TextField label="链接标题" value={registerInviteCodeHelpTitle} onChange={(value) => updateDraftField("registerInviteCodeHelpTitle", value)} placeholder="如何获得邀请码？" />
                <TextField label="链接地址" value={registerInviteCodeHelpUrl} onChange={(value) => updateDraftField("registerInviteCodeHelpUrl", value)} placeholder="如 /faq/invite-codes 或 https://example.com/help" />
              </div>
            ) : null}
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          ...
          <div className="rounded-[24px] border border-border p-5 space-y-4">
            ...
            <div className="grid gap-4 xl:grid-cols-3">
              ...
              <FieldGroup title="其它字段">
                ...
                <TextField label="昵称最小字符数" value={registerNicknameMinLength} onChange={(value) => updateDraftField("registerNicknameMinLength", value)} placeholder="如 1" />
                <TextField label="昵称最大字符数" value={registerNicknameMaxLength} onChange={(value) => updateDraftField("registerNicknameMaxLength", value)} placeholder="如 20" />
      ...
      {mode === "registration" && activeSubTab === "email-templates" ? (
...
function CaptchaModeField({ label, value, onChange }: { label: string; value: "OFF" | "TURNSTILE" | "BUILTIN" | "POW"; onChange: (value: "OFF" | "TURNSTILE" | "BUILTIN" | "POW") => void }) {
  return (
    <div className="space-y-2">
      <p className="text-sm font-medium">{label}</p>
      <select value={value} onChange={(event) => onChange(event.target.value as "OFF" | "TURNSTILE" | "BUILTIN" | "POW")} className="h-11 w-full rounded-full border border-border bg-background px-4 text-sm outline-hidden">
