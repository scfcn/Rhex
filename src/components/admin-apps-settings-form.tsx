  ...
  return (
    ...
    >
      <div className="rounded-[24px] border border-border p-5 space-y-4">
        <div>
          <h3 className="text-sm font-semibold">PC 搜索框应用入口</h3>
          <p className="mt-1 text-xs leading-6 text-muted-foreground">配置 PC 端搜索框左侧应用图标及下拉菜单项。支持站内路径与完整外链，前台会自动读取并展示。</p>
        </div>

        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <div className="space-y-2">
            <p className="text-sm font-medium">触发图标</p>
            <select value={headerAppIconName} onChange={(event) => setHeaderAppIconName(event.target.value)} className="h-11 w-full rounded-full border border-border bg-background px-4 text-sm outline-none">
        ...
        <div className="space-y-3">
          {links.map((item, index) => (
            <div key={`${item.id}-${index}`} className="rounded-[20px] border border-border p-4">
      ...
      <div className="flex items-center gap-3">
        <Button disabled={isPending}>{isPending ? "保存中..." : "保存应用入口"}</Button>
