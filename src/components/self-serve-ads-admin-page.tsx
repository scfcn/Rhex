    ...
    <div className="space-y-6">
      ...
      <section className="rounded-[24px] border border-border bg-card p-5 space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <Field label="侧栏插槽">
            <select value={state.sidebarSlot} onChange={(event) => state.setSidebarSlot(event.target.value as "home-right-top" | "home-right-middle" | "home-right-bottom")} className="h-11 w-full rounded-[16px] border border-border bg-background px-4 text-sm outline-none">
              ...
              <article key={order.id} className="rounded-[18px] border border-border bg-background/70 p-4">
                ...
                {expanded ? (
                  <div className="mt-4 grid gap-4 rounded-[18px] border border-border bg-card p-4 lg:grid-cols-2">
                    ...
                    <Field label="购买时长">
                      <select value={String(order.durationMonths ?? 1)} onChange={(event) => updateOrder(order.id, { durationMonths: Number(event.target.value) })} className="h-11 w-full rounded-[16px] border border-border bg-background px-4 text-sm outline-none">
                    ...
                    <label className="lg:col-span-2 block space-y-2">
                      ...
                      <textarea value={order.reviewNote ?? ""} onChange={(event) => updateOrder(order.id, { reviewNote: event.target.value })} className="min-h-[96px] w-full rounded-[16px] border border-border bg-background px-4 py-3 text-sm outline-none" />
                    </label>
...
function Field({ label, value, onChange, className = "", children }: { label: string; value?: string; onChange?: (value: string) => void; className?: string; children?: React.ReactNode }) {
  return (
    <label className={`block space-y-2 ${className}`.trim()}>
      ...
      {children ?? <input value={value} onChange={(event) => onChange?.(event.target.value)} className="h-11 w-full rounded-[16px] border border-border bg-background px-4 text-sm outline-none" />}
    </label>
  )
}
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <input value={value} onChange={(event) => onChange(event.target.value)} className="h-10 w-32 rounded-full border border-border bg-background px-3 text-sm outline-none" />
