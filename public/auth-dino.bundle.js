(() => {
  'use strict';
    ...
    */
    ...
    this.startListening();
    this.update();
    this.resizeHandler = this.resizeHandler || this.debounceResize.bind(this);
    ...
    */
    ...
    this.startGameHandler = this.startGameHandler || this.startGame.bind(this);
    ...
    */
    ...
    this.containerEl.removeEventListener(Runner.events.ANIM_END, this.startGameHandler);
    }
    this.runningTime = 0;
    ...
    this.visibilityChangeHandler = this.visibilityChangeHandler || this.onVisibilityChange.bind(this);
    ...
    */
    ...
    if (e.type == Runner.events.KEYDOWN) {
    e.preventDefault();
    }
    if (!this.activated) {
    this.loadSounds();
    ...
    */
    ...
    cancelAnimationFrame(this.raqId);
    this.raqId = 0;
    if (this.startGameHandler && this.containerEl) {
    this.containerEl.removeEventListener(Runner.events.ANIM_END, this.startGameHandler);
    }
    if (this.resizeHandler) {
    window.removeEventListener(Runner.events.RESIZE, this.resizeHandler);
    }
    if (this.visibilityChangeHandler) {
    window.removeEventListener(Runner.events.VISIBILITY, this.visibilityChangeHandler);
    window.removeEventListener(Runner.events.BLUR, this.visibilityChangeHandler);
    window.removeEventListener(Runner.events.FOCUS, this.visibilityChangeHandler);
    }
  ...
  function mount(host) {
    ...
    if (window.Runner && window.Runner.instance_ && typeof window.Runner.instance_.destroy === 'function') {
      window.Runner.instance_.destroy();
    }

    if (!host.id) {
      host.id = 'auth-dino-' + Math.random().toString(36).slice(2);
    }

    host.classList.add('offline');
    host.setAttribute('aria-label', 'Chrome Dino game');
    host.innerHTML = resourcesMarkup;

    const runner = new Runner('#' + host.id);
    let disposed = false;
